...
```bash
aws elbv2 delete-load-balancer --load-balancer-arn $Internal_LoadBalancerArn --region $AWS_REGION
aws elbv2 delete-target-group --target-group-arn $nodejs_TargetGroupArn --region $AWS_REGION
aws elbv2 delete-target-group --target-group-arn $crystal_TargetGroupArn --region $AWS_REGION
